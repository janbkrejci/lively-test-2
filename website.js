'format esm';
import {
  Morph, /* Image, */ TilingLayout, StyleRules,
...
const pfx = '/hornung/'; const duration = 300; const easing = null; // Expo.easeOut;
...
async function fitToContainer (container, background, animated) {
  let ext = background._natExt = background._natExt || await background.naturalExtent();
  let { width, height } = pt(0, 0).extent(ext).scaleRectIn(container.innerBounds());
  let extent = ext.scaleBy(1 / Math.min(width, height));
  let center = container.innerBounds().center();
  let bounds = pt(0, 0).extent(extent).withCenter(center);
  if (animated) {
...
async function fitToContainerHeight (container, background) {
  let ext = background._natExt = background._natExt || await background.naturalExtent();
  let { width, height } = pt(0, 0).extent(ext).scaleRectIn(container.innerBounds());
  let extent = ext.scaleBy(1 / height);
  let center = container.innerBounds().center();
  let bounds = pt(0, 0).extent(extent).withCenter(center);
  background.setBounds(bounds);
}
...
Alles singt bei ihm."`,
  ...
  get styler () {
    return new StyleRules({
      ...
      mediaBackground: {
        fill: new LinearGradient({ stops: [{ color: Color.transparent, offset: 0 }, { color: Color.black, offset: 1 }] }),
        clipMode: 'hidden'
      },
      titleLabel: {
        readOnly: true,
  ...
  relayout () {
    ...
    const title = this.get('pageTitle');
    ...
    let delta = this.innerBounds().topRight().subPt(this.innerBounds().center());
...
export class Body extends Morph {
  ...
  renderTriebwerk (extent, position) {
    ...
    const triebwerkSC = triebwerkTunes.map(src => this.renderSoundCloudTune(src));
    const jt = morph({
      type: 'html',
      fill: Color.transparent,
      html: renderParagraph(jazzthing),
    ...
    const selector = morph(({
      ...
      items: {
        ...
        'Jazzpodium, 02/17': renderParagraph(podium)
      },
      init: 'Jazz thing, 02/17'
    }));
    const pressHolder = morph({
      position: pt(20, 50),
      showArticle (article) {
    ...
    const content = morph({
      ...
      async updateExtent (ext) {
        ...
        const bg = this.get('mediaBackground');
        const tunes = this.get('tunes');
        this.extent = ext;
        bg.extent = ext;
      ...
      submorphs: [
        ...
        {
          type: 'text',
          textString: 'Triebwerk Hornung',
          morphClasses: ['projectTitle'],
          position: pt(25, 30)
        },
        {
          fill: Color.transparent,
  ...
  renderTrio (extent, position) {
    ...
    const trioYT = trioVideos.map(src => this.renderYoutubeVideo(src));
    const content = morph({
      fill: Color.transparent,
      visible: false,
      updateExtent (ext) {
        ...
        },
        {
          ...
          submorphs: [{
            ...
            submorphs: [{
              position: pt(20, 0),
              type: 'html',
              html: renderParagraph(trioDescription),
              fill: Color.transparent
            }]
          }]
        },
  ...
  renderCD (extent, position) {
    ...
    const content = morph({
      ...
      submorphs: [{
        ...
        type: 'html',
        name: 'itunesLink',
        center: pt($world.width * .75, $world.height * .3),
        fill: Color.transparent,
      ...
      },
      {
        type: 'html',
        name: 'amazonLink',
        center: pt($world.width * .75, $world.height * .4),
        fill: Color.transparent,
